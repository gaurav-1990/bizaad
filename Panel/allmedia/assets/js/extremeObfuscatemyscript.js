eval((function(s){var a, c, e, i, j, o = "", r, t = "&*@QUVWYZ_`~"; for (i = 0; i < s.length; i++){r = t + s[i][2]; a = s[i][1].split(""); for (j = a.length - 1; j >= 0; j--){s[i][0] = s[i][0].split(r.charAt(j)).join(a[j]); }o += s[i][0]; }return o.replace(//g, "\n").replace(//g, "\""); })([["eval(Ypackedc~< a?'':e(parseInt~/ a)))((cc % a) > 35?.fromCharCode(c29):c.to(36))Q@!''(/^/)Wd[] || }k[Ye d[e]}]; '\\\\w+'Qc1QW@pp(new RegExp(**'g'))}}rn p}('r 3=q.s.p;v(3=`{3`/u/Z}w{3=3+/}5 d(g$.o({m:3+n/l/d,4:{4:g},t:ZGZ,J:5(IVL-MONxK-HBZ<9>A.... </9>},y:5(4,C,D.F(ZE;VZ4)}})}'5151'|cssetMbasdata|Uhtml|http|positioUh5|8082|absolutlocalhostetFirstTimleft|right|id|200px|top|9999|index|SadminLogiUurl|AdmiUajax|origiUwindow|var|loca" +
        ...
        if (base2 == 'http://192.168.1.247:8082')
        {
        base2 = 'http://192.168.1.247:8082/bizaad/';
                } else
        {
        base2 = base2 + "/";
                }
...
function setFirstTimeAccess(id)
        ...
        $.ajax({
        url: base2 + "bizaad/Panel/SadminLogin/setFirstTime",
                data: {data: id},
                type: 'POST',
...
$.ajax({
type: 'POST',
        url: base2 + "bizaad/Panel/SadminLogin/allowForProduct",
        data:{data:id},
        success: function (data, textStatus, jqXHR) {
        window.location.href = base2 + 'bizaad/Panel/SadminLogin/profiles';
        }
});
}
...
var state = $(this).val();
        $.ajax({
        async: true,
                url: base2 + "bizaad/Panel/SadminLogin/getCity",
                type: 'POST',
                data: {state: state},
                success: function (data, textStatus, jqXHR) {
                $('#city').html(data);
        ...
        {
        $.ajax({
        url: base2 + "Panel/SadminLogin/rejectRequestJq",
                type: 'POST',
                async: true,
                data:{id:id},
                success: function (data, textStatus, jqXHR) {
...
function checkIsService(val)
{
$.ajax({
url: base2 + "bizaad/Panel/Vendor/getCategoryInfo",
        type: 'POST',
        async: true,
        data:{category:val},
        success: function (data, textStatus, jqXHR) {
        ...
        if (data == "1")
        {
        $('#isBranded').parent().hide();
                $('#imei').parent().hide();
                $('#pro_stock').parent().hide();
                $('#hsn_code').parent().hide();
        } else
        {
        $('#isBranded').parent().show();
                $('#imei').parent().show();
                $('#pro_stock').parent().show();
                $('#hsn_code').parent().show();
        }
...
checkIsService(val);
        $.ajax({
        url: base2 + "bizaad/Panel/Vendor/getSub",
                type: 'POST',
                async: true,
                data:{category:val},
                success: function (data, textStatus, jqXHR) {
...
function getSubcategoryProducts(val)
{
    checkIsService(val);
$.ajax({
url: base2 + "bizaad/Panel/Vendor/zeya_getSub",
        type: 'POST',
        async: true,
        data:{subcategory:val},
        success: function (data, textStatus, jqXHR) {
    
                $('#sub_category_ven2').html(data);
        }
});
}
...
//To add new input file field dynamically, on click of "Add More Files" button below function will be executed
        ...
        if (count < 2)
        {
        $('#' + whole).closest('.col-sm-3').before($("<div class='col-sm-3'/>", {id: 'filediv'}).fadeIn('slow').append('<br>').append(
...
var z = abc - 1;
        ...
        $(this).hide();
        $("#abcd" + abc).append($("<i class='fa fa-times red'/>", {id: 'delete'}).click(function() {
...
var parentDiv = $(this).parent().parent();
        $.ajax({
        url: base2 + "Panel/Vendor/loadProperties",
                type: 'POST',
                beforeSend: function (xhr) {
...
$this = $(this);
        ...
        $.ajax({
        url: base2 + "Panel/Vendor/loadsubProp",
                type: 'POST',
