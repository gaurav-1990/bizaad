...
<html>
    <head>
        <title>Invoice 00<?= $data[0]->invoiceid ?></title>
        <meta charset="UTF-8">
			    ...
			    <button class="btn btn-dark"  onclick="window.print();">Print</button>
                        ...
                        <div class="inv-logo">
                            <a target="_blank" href="#">
                                ...
                                <img src="<?=  base_url()?>allmedia/images/logo.png" style="width: 85%;" alt=""/>
                            </a>
                            <p>
                        ...
                        <div class=" company-details">
                            ...
                            <div class="gst-no">ORDER NO.: <span><?= $invoice->or_id ?></span></div>
                            <div class="gst-no">INVOICE NO.: <span><?= $invoice->invoiceid ?></span></div>
                    ...
                    <main>
                        <div class=" contacts bill-set">
                            <div class=" invoice-to">
                                <div class="bill-name">BILL TO: <span><?= ucfirst($invoice->first_name) ?> <?= ucfirst($invoice->last_name) ?></span></div>
                                <div class="address"><?= ucfirst($invoice->user_address) ?>, <?= ucfirst($invoice->cus_state) ?></div>
                                <span class="state">
                                    <a >State</a> <span><?= ucfirst($invoice->cus_state) ?></span>
                                </span>
                                <span class="state">
                                    <a >Pin</a> <span><?= ucfirst($invoice->user_pin_code) ?></span>
                                </span>
                            </div>
			   <?php
			       if(isset($invoice->ven_id) && $invoice->ven_id !=''){
				   $getVendor = getVendor($invoice->ven_id);				   
			    ...
			    <?php
				   ...
				   ?>
                            <div class="invoice-details">
                                <div class="bill-name">SHIP TO: <span><?= ucfirst($invoice->first_name) ?> <?= ucfirst($invoice->last_name) ?></span></div>
                        ...
                        <div class="tab-block">
                            <table border="0" style="border-collapse: collapse;width: 100%" cellspacing="0" cellpadding="0">
                                ...
                                <tbody>
                                    ...
                                    $total = 0.0;
                                    foreach ($data as $in) {
					$getVendorInfo = getVendorPro($in->ven_id,$in->pro_id);
//					echo '<pre>';
//					print_r($getVendorInfo);
//					echo '</pre>';
					if(isset($in->ven_id) && $in->ven_id !=''){
					$grand = (floatval($getVendorInfo->dis_price) * floatval($in->pro_qty));
                                        $total = $total + $grand;
					}else{
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-left">
                                                <h3>
                                                    ...
                                                    <?php
//                                                    if ($in->property != '') {
//                                                        foreach (json_decode($in->property) as $prop) {
//                                                            echo "<small><br>";
//                                                            echo $prop->prop;
//                                                            echo ":";
//                                                            echo $prop->val."</small>";
...
//                                                    }
					    ...
					    <?php if(isset($in->ven_id) && $in->ven_id !=''){ ?>
                                            <td class="total"><?= number_format($getVendorInfo->dis_price, 2, '.', ''); ?></td>
					    <?php }else{?>
					     <td class="total"><?= number_format($in->dis_price, 2, '.', ''); ?></td>
					    <?php } ?>
                                            <?php
						if(isset($in->ven_id) && $in->ven_id !=''){ 
						    $taxable = (($grand * 100) / ((100) + floatval($getVendorInfo->gst_pro)));
						}  else {
					    ...
					    <?php if(isset($in->ven_id) && $in->ven_id !=''){ ?>
                                            <td class="total"><?= number_format(floatval($getVendorInfo->dis_price) * floatval($in->pro_qty), 2); ?></td>
					    <?php }else{?>
                        ...
                        <div class="value">
                            <p>INVOICE VALUE (in words) <span>     <?= ucwords(numberTowords($total)) ?></span></p>
                        </div>
                        <div class="notices">
